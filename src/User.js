...
const User = () => {
    ...
    const { aboutMe } = useParams('aboutMe');
    const { linkedin } = useParams('linkedin');
    const { github } = useParams('github');

    return(
        <div id='background'>
            <div id='main'>
                <div className="col1">
                    <div id='name'> {name.replace(/_/g, ' ')} </div> 
                    <br></br>
                    <div id='aboutMe'> {aboutMe.replace(/_/g, ' ')} </div> 
                    <br></br>
                    <div class="social-links">
                        <a href={`https://www.linkedin.com/${linkedin.replace('_', '/')}`}>
